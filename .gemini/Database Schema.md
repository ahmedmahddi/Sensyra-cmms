...
This document defines the complete PostgreSQL database schema using Prisma ORM. The schema is designed for:
...
model Attachment {
  ...
  entityType      String       // WorkOrder, WorkRequest, Asset, etc.
  entityId        String
  ...
  workOrder       WorkOrder?   @relation(fields: [entityId], references: [id], onDelete: Cascade)
  workRequest     WorkRequest? @relation(fields: [entityId], references: [id], onDelete: Cascade)
...
model Comment {
  ...
  // Polymorphic
  entityType    String    // WorkOrder, Asset, etc.
  entityId      String
...
CREATE INDEX idx_work_orders_org_status ON "WorkOrder"("organizationId", "status");

-- Work orders by assignee
CREATE INDEX idx_work_orders_assigned ON "WorkOrder"("assignedToId") WHERE "deletedAt" IS NULL;

-- Work orders by due date
CREATE INDEX idx_work_orders_due_date ON "WorkOrder"("dueDate") WHERE "status" != 'COMPLETED';

-- Assets by location
CREATE INDEX idx_assets_location ON "Asset"("locationId", "status");

-- PM schedules due soon
CREATE INDEX idx_pm_next_due ON "PMSchedule"("nextDueDate") WHERE "isActive" = true;

-- Full-text search on work orders
CREATE INDEX idx_work_orders_search ON "WorkOrder" USING gin(to_tsvector('english', title || ' ' || COALESCE(description, '')));
```
...
**Next**: See `03-API_SPECIFICATIONS.md` for API endpoints